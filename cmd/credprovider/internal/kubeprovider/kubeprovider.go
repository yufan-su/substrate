...
// atespace's allowed list. Only the egress-injection scope is authorized today;
// any other scope is denied so a future scope cannot resolve secrets before its
// authorization story is defined.
func (s *Server) authorize(ctx context.Context, reqCtx *credproviderpb.SecretRequestContext, namespace string) error {
	if s.nsAuth == nil {
		return nil
	}
	if reqCtx.GetScope() != credproviderpb.SecretScope_SECRET_SCOPE_EGRESS_CREDENTIAL_INJECTION {
		return status.Errorf(codes.PermissionDenied, "authorization not configured for scope %s", reqCtx.GetScope())
	}
