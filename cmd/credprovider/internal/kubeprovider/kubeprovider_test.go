...
func TestRequestSecretAuthorization(t *testing.T) {
	...
	egress := credproviderpb.SecretScope_SECRET_SCOPE_EGRESS_CREDENTIAL_INJECTION
	...
	tests := []struct {
		...
		{
			name: "allowed",
			ctx:  &credproviderpb.SecretRequestContext{Scope: egress, ActorIdentity: teamAURI},
		...
		},
		{
			...
			ctx:      &credproviderpb.SecretRequestContext{Scope: egress, ActorIdentity: teamBURI},
			uri:      "substrate-secret://kubernetes.io/p/ns1/example-api/token",
			wantCode: codes.PermissionDenied,
		},
		{
			name:     "garbage identity",
			ctx:      &credproviderpb.SecretRequestContext{Scope: egress, ActorIdentity: "not-a-spiffe-uri"},
			uri:      "substrate-secret://kubernetes.io/p/ns1/example-api/token",
			wantCode: codes.PermissionDenied,
		},
		{
			name:     "wrong scope",
			ctx:      &credproviderpb.SecretRequestContext{Scope: credproviderpb.SecretScope_SECRET_SCOPE_UNSPECIFIED, ActorIdentity: teamAURI},
			uri:      "substrate-secret://kubernetes.io/p/ns1/example-api/token",
			wantCode: codes.PermissionDenied,
		},
	}
	...
	t.Run("nil authorizer bypasses", func(t *testing.T) {
		...
		if _, err := srv.RequestSecret(context.Background(), &credproviderpb.RequestSecretRequest{
			Uri:     "substrate-secret://kubernetes.io/p/ns1/example-api/token",
			Context: &credproviderpb.SecretRequestContext{Scope: egress, ActorIdentity: "not-a-spiffe-uri"},
		}); err != nil {
