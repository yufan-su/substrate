...
func TestRequestSecret(t *testing.T) {
	...
	// The stored secret is a host-keyed credential set; the provider returns the
	// value for the request's (destination, header).
	const set = `{"api.example.com": {"Authorization": "Bearer s3cr3t"}}`
	tests := []struct {
		name     string
		...
		{
			name:     "found",
			accessor: &fakeAccessor{byName: map[string][]byte{name: []byte(set)}},
			uri:      "substrate-secret://secretmanager.googleapis.com/projects/proj-123/secrets/example-api",
		...
		},
		{
			...
			accessor: &fakeAccessor{byName: map[string][]byte{"projects/proj-123/secrets/example-api/versions/5": []byte(`{"api.example.com": {"Authorization": "v5"}}`)}},
			uri:      "substrate-secret://secretmanager.googleapis.com/projects/proj-123/secrets/example-api/versions/5",
			dest:     "api.example.com",
			header:   "Authorization",
			want:     "v5",
		...
		{
			name:     "no value for destination is empty",
			accessor: &fakeAccessor{byName: map[string][]byte{name: []byte(set)}},
			uri:      "substrate-secret://secretmanager.googleapis.com/projects/proj-123/secrets/example-api",
			dest:     "other.example.com",
			header:   "Authorization",
			want:     "",
		},
		{
			name:     "malformed set fails closed",
			accessor: &fakeAccessor{byName: map[string][]byte{name: []byte("not-json")}},
			uri:      "substrate-secret://secretmanager.googleapis.com/projects/proj-123/secrets/example-api",
			dest:     "api.example.com",
			header:   "Authorization",
		...
		},
		{
			...
			accessor: &fakeAccessor{byName: map[string][]byte{name: []byte(set)}, corrupt: true},
			uri:      "substrate-secret://secretmanager.googleapis.com/projects/proj-123/secrets/example-api",
			wantCode: codes.Unavailable,
		},
		{
