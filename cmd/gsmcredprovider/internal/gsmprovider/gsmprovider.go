...
import (
	...
	"github.com/agent-substrate/substrate/cmd/gsmcredprovider/internal/credset"
...
func (s *Server) RequestSecret(ctx context.Context, req *credproviderpb.RequestSecretRequest) (*credproviderpb.RequestSecretResponse, error) {
	...
	slog.InfoContext(ctx, "resolving credential",
		...
		slog.String("header", req.GetContext().GetHeaderKey()),
	)
	...
	// The stored secret is a host-keyed credential set; return just the value for
	// the requested header at the destination. A malformed set fails closed; a set
	// with no such entry returns an empty secret so the injector injects nothing.
	value, err := credset.ValueFor(payload.GetData(), req.GetContext().GetDestination(), req.GetContext().GetHeaderKey())
	if err != nil {
		return nil, status.Errorf(codes.Internal, "secret version %s: %v", ref.ResourceName(), err)
	}
