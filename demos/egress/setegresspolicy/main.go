...
// with a single hostname rule that injects one header, via the ateapi Control
// API. It reuses kubectl-ate's client (auto port-forward + a Kubernetes
// ServiceAccount bearer token), so it needs no client certificate.
//
// The policy names the header (and optional prefix) to inject; the provider
// resolves that header's value for the destination from the credential the URI
// names.
//
...
func main() {
	var (
		...
		header      = flag.String("header", "", "HTTP header to inject, e.g. Authorization (required)")
		prefix      = flag.String("prefix", "", "prefix prepended to the resolved value, e.g. 'Bearer '")
		uri         = flag.String("uri", "", "credential substrate-secret:// URI (required)")
		replace     = flag.Bool("replace", false, "delete an existing policy first, then create (for iterating)")
	)
	flag.Parse()

	missing := false
	for name, v := range map[string]string{"atespace": *atespace, "actor": *actor, "host": *host, "header": *header, "uri": *uri} {
		if v == "" {
