...
whole run, doing two things:
  ...
  * `ActorPing` on every iteration — a round trip through the router to the
    actor and back, covering the data path (router -> worker -> sandbox ->
    actor).
  * `ActorSuspend` / `ActorResume` every `--soak-pings-per-cycle` iterations —
    a full suspend/resume, covering the lifecycle path (checkpoint upload,
...
class SoakUser(User):
    ...
    def cycle_actor(self) -> None:
        ...
        later resume restores from the snapshot store instead — so it is the
        ...
        if not self._timed_transition(
            ...
            # which case pings still work and the next cycle can retry — do not
            # throw away a healthy user over it. Resuming here would be worse
            # than useless: ResumeActor on a RUNNING actor fails, and the user
            # would be aborted for a fault it does not have.
        ...
        if not self._timed_transition(
            ...
            self._abort("did not come back from its suspend/resume cycle")
    ...
    def _timed_transition(self, name: str, rpc_name: str, invoke, target: int) -> bool:
        ...
        accepting the request, which for suspend and resume returns long before
        the work is done. The row fired here under `name` is the wall-clock to
        actually reach `target`, checkpoint upload or snapshot restore
        included. The second one is what drifts.
        """
